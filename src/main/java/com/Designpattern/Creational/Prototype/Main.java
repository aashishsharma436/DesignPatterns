...
public class Main {
    /*
    1.  This Design pattern is used when object creation is a time-consuming and costly operation. So, we create objects with the existing
        objects itself by copying the existing ones.
    ...
    3.  One of the best available ways to create an object from existing objects is the clone() method. Clone is the simplest approach to
        implementing a prototype pattern. However, it is your call to decide how to copy existing objects based on your business model.
     */
...
3. Consider the prototype pattern for dynamic configurations where you need to create objects at runtime. You can clone a base configuration
...
2. If object creation is simple and does not involve significant resource consumption, and there are no variations of objects, using the
   prototype pattern might be an unnecessary complexity.
